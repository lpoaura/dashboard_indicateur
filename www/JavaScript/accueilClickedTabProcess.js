/* Ce script js permet de récupérer l'id de l'onglet du mode Accueil sélectionné
par l'utilisateur via un clic*/

$(document).ready(function () {

  console.log("JQuery is ok");
  
  $(function () {
    const onClick = (event) => {
      /* Si on clique sur un bouton qui n'est pas un bouton autre que ceux du mode Accueil ou si on clique sur la div que contient un bouton d'un indicateur général multiple*/
      if ((event.target.nodeName === 'BUTTON' && event.target.className!=="btn btn-default action-button shiny-bound-input" && event.target.className!=="btn btn-default action-button") || event.target.className === 'labelMultipleInd' || event.target.parentElement.className === 'labelMultipleInd') {
        /*On affiche la valeur dans la console du browser*/
        console.log(event.target.id);
        /*On modifie un input R qui va être écouté par un ObserverEvent*/
        if(event.target.parentElement.className === 'labelMultipleInd'){
          Shiny.setInputValue('tabAccueilNew', event.target.parentElement.id,{priority: "event"});
        }
        else{
          Shiny.setInputValue('tabAccueilNew', event.target.id,{priority: "event"});
        }
      }
    }
    window.addEventListener('click', onClick);
    });
  });
